...
class MainWindow(QMainWindow):
    ...
    def _emit_auth_error_async(self) -> None:
        """Llamado desde el worker thread — emite señal Qt para cruzar al hilo principal."""
        self._auth_error.emit()

    @Slot()
    def _on_auth_error(self) -> None:
        """Recibe error de autenticación en el hilo principal. Muestra error permanente."""
        log.error("API key de OpenAI inválida — extracción desactivada")
        self._set_status("Error: API key inválida — extracción desactivada", "warning")
